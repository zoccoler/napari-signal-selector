...
def load_flashing_polygons_data():
    ...
    if not extracted_folder_path.exists() or (
        ...
        if total_size != 0 and progress_bar.n != total_size:
            print(
                f"ERROR: Something went wrong with the download. File url: {zip_url}"
            )
    ...
    labels = imread(extracted_folder_path / "synthetic_labels.tif").astype(
        "uint32"
    )
    table = read_csv(
        extracted_folder_path / "table_synthetic_data_temporal_features.csv"
    )
    return [
        (timelapse, {"name": "Flashing Polygons Synthetic Timelapse"}),
        (
            labels,
            {
                "name": "Flashing Polygons Synthetic Labels",
                "features": table,
                "opacity": 0.4,
            },
            "labels",
...
def load_blinking_polygons_data():
    ...
    timelapse = imread(
        extracted_folder_path / "synthetic_timelapse_sub_signals.tif"
    )
    labels = imread(
        extracted_folder_path / "synthetic_labels_sub_signals.tif"
    ).astype("uint32")
    table = read_csv(
        extracted_folder_path
        / "table_synthetic_data_temporal_features_with_annotations_sub_signals.csv"
    )
    return [
        (timelapse, {"name": "Blinking Polygons Synthetic Timelapse"}),
        (
            labels,
            {
                "name": "Blinking Polygons Synthetic Labels Timelapse",
                "features": table,
                "opacity": 0.4,
            },
            "labels",
